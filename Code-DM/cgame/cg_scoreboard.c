...
#define SB_HEADER			105
#define SB_TOP				(SB_HEADER+32)
#define SB_HEADERTEXT		(SB_HEADER + 8)
#define SB_TOPLINE_LENGTH	590

// Where the status bar starts, so we don't overwrite it
#define SB_STATUSBAR		420

#define SB_NORMAL_HEIGHT_BIG	34
...
// Normal
#define SB_BOTICON_X		(SCOREBOARD_X+20)
#define SB_HEAD_X			(SCOREBOARD_X+50)
...
#define SB_SCORELINE_X_BIG	(SCOREBOARD_X		+ 125)
#define SB_NAME_X_BIG		(SB_SCORELINE_X_BIG + 12) 
#define SB_SCORE_X_BIG		(SB_SCORELINE_X_BIG + 200) 
#define SB_TIME_X_BIG		(SB_SCORELINE_X_BIG + 300) 
#define SB_PING_X_BIG		(SB_SCORELINE_X_BIG + 400) 
...
static void CG_NamePrep(char *buffer,char *name,int pixelCnt,int font)
{
	char str[2];
	int	length;
...
static void CG_DrawClientScore_Big( int y, score_t *score, float *color, float fade, qboolean largeFormat ) {
	...
	if (cgs.gametype < GT_TEAM)
	{
		...
		{
			CG_FillRect( SCOREBOARD_X, y,SB_TOPLINE_LENGTH , SB_NORMAL_HEIGHT_BIG, hcolor );
		}
	}
...
//				CG_DrawSmallStringColor( iconx, y, string, color );
	...
	if ( score->ping == -1 ) {
		...
		UI_DrawProportionalString( SB_NAME_X_BIG, y , string, UI_SMALLFONT, colorTable[CT_WHITE]);
	} 
	else if ( ci->team == TEAM_SPECTATOR ) 
	{
		Com_sprintf(string,sizeof(string),"(%s)%s",ingame_text[IGT_SPECTABBREV],ci->name);
		CG_NamePrep(string2,string,184,UI_SMALLFONT); //184 pixels in the name column
		UI_DrawProportionalString( SB_NAME_X_BIG, y , string2, UI_SMALLFONT, colorTable[CT_WHITE]);

		Com_sprintf(string,sizeof(string),"%5i",score->score);
		UI_DrawProportionalString( SB_SCORE_X_BIG, y ,  string, UI_SMALLFONT, colorTable[CT_YELLOW]);

		Com_sprintf(string,sizeof(string),"%5i",score->time);
		UI_DrawProportionalString( SB_TIME_X_BIG, y ,  string, UI_SMALLFONT, colorTable[CT_LTGOLD1]);

		Com_sprintf(string,sizeof(string),"%5i",score->ping);
		UI_DrawProportionalString( SB_PING_X_BIG, y,  string, UI_SMALLFONT, colorTable[CT_LTGOLD1]);
	} 
	else 
	{
		CG_NamePrep(string,ci->name,184,UI_SMALLFONT); //184 pixels in the name column
...
//				CG_DrawSmallStringColor( iconx, y, string, color );
	...
	VectorClear( headAngles );
	headAngles[YAW] = 180;
	CG_DrawHead( SB_HEAD_X+14, y, picSize, picSize, score->client, headAngles );
...
qboolean CG_DrawScoreboard( void ) 
{
	...
	if ( !inIntermission && cg.snap->ps.persistant[PERS_TEAM]!=TEAM_SPECTATOR) 
	{
		...
		if ( cg.killerName[0] ) 
		{
			...
			UI_DrawProportionalString((SCREEN_WIDTH/2), 75, s, UI_BIGFONT|UI_CENTER, colorTable[CT_CYAN]);
		}
	}
	// scoreboard
	y = SB_HEADER;
...
//	CG_DrawPic( SCOREBOARD_X, y, 16, 32, cgs.media.corner_12_24 );	// Corner
	...
	{
		UI_DrawProportionalString( SB_NAME_X_BIG, SB_HEADERTEXT, ingame_text[IGT_SB_NAME],UI_SMALLFONT, colorTable[CT_BLACK] );
		UI_DrawProportionalString( SB_SCORE_X_BIG, SB_HEADERTEXT, ingame_text[IGT_SB_SCORE],UI_SMALLFONT, colorTable[CT_BLACK] );
		UI_DrawProportionalString( SB_TIME_X_BIG, SB_HEADERTEXT, ingame_text[IGT_SB_TIME],UI_SMALLFONT, colorTable[CT_BLACK] );
		UI_DrawProportionalString( SB_PING_X_BIG, SB_HEADERTEXT, ingame_text[IGT_SB_PING],UI_SMALLFONT, colorTable[CT_BLACK] );
	}
	...
	if (cgs.gametype < GT_TEAM)
	{
		...
		hcolor[2] = 0;
		hcolor[3] = fade * 0.7;
		CG_FillRect( SCOREBOARD_X, y + 24,SB_TOPLINE_LENGTH, 8, hcolor );
	}
	...
	CG_FillRect( 13 + 5, y, SB_TOPLINE_LENGTH - 5, 24, colorTable[CT_DKORANGE]);
	trap_R_SetColor( colorTable[CT_DKORANGE] );
	CG_DrawPic( 605, y,  16, 32, cgs.media.scoreboardEndcap );
	...
	UI_DrawProportionalString( SCOREBOARD_X+10, y+6, gamename, 0, colorTable[CT_BLACK]);
