	...
	else
	{
		...
		if (s->listnames)
		{
			if ( !strchr( menu_normal_text[s->listnames[s->curvalue]], '\n' ) )
			{
				...
				if (!s->listX && !s->listY)
				{
					listX = x + MENU_BUTTON_MED_HEIGHT + boxWidth - 8 + MENU_BUTTON_MED_HEIGHT + 4;
					UI_DrawProportionalString( listX, y + s->textY,menu_normal_text[s->listnames[s->curvalue]], UI_SMALLFONT, colorTable[color] );
				}
				else
				{
					UI_DrawProportionalString( x+ s->listX, y + s->listY,menu_normal_text[s->listnames[s->curvalue]], UI_SMALLFONT, colorTable[color] );
				}
			}
		}
		else if (s->itemnames[0])
		{
			if ( !strchr(s->itemnames[s->curvalue], '\n' ) )
			{
				...
				{
					listX = x + MENU_BUTTON_MED_HEIGHT + boxWidth - 8 + MENU_BUTTON_MED_HEIGHT + 4;
					UI_DrawProportionalString( listX, y + s->textY,s->itemnames[s->curvalue], UI_SMALLFONT, colorTable[color] );
				}
				else
				{
					UI_DrawProportionalString( x+ s->listX, y + s->listY,s->itemnames[s->curvalue], UI_SMALLFONT, colorTable[color] );
				}
			}
		}
	}
