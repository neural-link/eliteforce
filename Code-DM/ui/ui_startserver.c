...
void StartServer_Graphics (void)
{
	...
	UI_DrawHandlePic(30,203, 47, 106, uis.whiteShader);	// Left side of frame
	UI_DrawHandlePic(30,312, 47, 106, uis.whiteShader);	// Left side of frame
	...
	UI_DrawHandlePic(x+ 81, 320,  32, 32,s_startserver.corner_ll);
	UI_DrawHandlePic(x+369, 320,  32, 32,s_startserver.corner_lr);

	UI_DrawHandlePic(x+ 81, 64, 18, 252, uis.whiteShader);	// Left side line
	UI_DrawHandlePic(x+373, 64, 18, 252, uis.whiteShader);	// Right side line
	UI_DrawHandlePic(x+81, 56, 310, 8, uis.whiteShader);		// Top

	UI_DrawHandlePic(x+94, 324,  15, 18, uis.whiteShader);	// Bottom
	UI_DrawHandlePic(x+112, 324,  22, 18, uis.whiteShader);	// Bottom
	UI_DrawHandlePic(x+137, 324, 198, 18, uis.whiteShader);	// Bottom
	UI_DrawHandlePic(x+338, 324,  22, 18, uis.whiteShader);	// Bottom
	UI_DrawHandlePic(x+364, 324,  21, 18, uis.whiteShader);	// Bottom

	UI_DrawProportionalString(  74,  30, "6801",UI_RIGHT|UI_TINYFONT, colorTable[CT_BLACK]);
	UI_DrawProportionalString(  74, 153, "676",UI_RIGHT|UI_TINYFONT, colorTable[CT_BLACK]);
	UI_DrawProportionalString(  74, 209, "655",UI_RIGHT|UI_TINYFONT, colorTable[CT_BLACK]);
	UI_DrawProportionalString(  74, 317, "615222",UI_RIGHT|UI_TINYFONT, colorTable[CT_BLACK]);
...
//	x = 202;
	...
	s_startserver.prevpage.generic.x		= START_X_POS + 114;
	s_startserver.prevpage.generic.y		= 325;
	...
	s_startserver.nextpage.generic.x		= START_X_POS + 342;
	s_startserver.nextpage.generic.y		= 325;
...
static void ServerOptions_LevelshotDraw( void *self ) {
	...
	UI_DrawHandlePic(  (s_serveroptions.mappic.generic.x - 2), (s_serveroptions.mappic.generic.y - 2), (s_serveroptions.mappic.width + 4),  164, uis.whiteShader);
...
static void ServerOptions_InitBotNames( void ) {
	...
	if( s_serveroptions.gametype >= GT_TEAM ) {
		...
		Q_strncpyz( s_serveroptions.playerNameBuffers[8], "2_of_3 ", 16 );
		if( s_serveroptions.gametype == GT_TEAM ) {
...
static void ServerOptions_MenuInit( qboolean multiplayer ) 
{
	...
	s_serveroptions.mappic.generic.y			= 83;
...
static void AdvancedServer_Event( void* ptr, int notification ) 
{
	...
	case ID_PMOD_ASSIMILIATION:
		...
		if ((trap_Cvar_VariableValue( "g_pModAssimilation" )) || (trap_Cvar_VariableValue( "g_pModElimination" )))
		{
			s_advancedserver.nojointimeout.generic.flags	&= ~QMF_HIDDEN;
		}
		else
		{
			s_advancedserver.nojointimeout.generic.flags	|= QMF_HIDDEN;
...
void M_AdvancedServerMenu_Graphics(void)
{
	...
	case ERR_DOWARMUP :
		...
		break;
	case ERR_NOJOINTIMEOUT :
		s_advancedserver.errorText.generic.flags &= ~QMF_HIDDEN;
		s_advancedserver.errorText.generic.y = s_advancedserver.nojointimeout.generic.y;
		s_advancedserver.errorText.generic.x = s_advancedserver.nojointimeout.generic.x + ERROR_X_OFFSET;
		break;
...
static void ServerAdvanced_StatusBar( void* ptr ) 
{
	...
	case ID_DOWARMUP :
		...
		break;
	case ID_NOJOINTIMEOUT:
		UI_DrawProportionalString( x, y, menu_normal_text[MNT_ADV_STATUS1_NOJOINTIMEOUT], UI_SMALLFONT, colorTable[CT_WHITE]);
		UI_DrawProportionalString( x, (y+ PROP_HEIGHT + 2), menu_normal_text[MNT_ADV_STATUS2_NOJOINTIMEOUT], UI_SMALLFONT, colorTable[CT_WHITE]);
		break;
...
*/
	...
	y += pad;
	s_advancedserver.nojointimeout.generic.type					= MTYPE_FIELD;
	...
	s_advancedserver.nojointimeout.field.maxchars				= 4;
	s_advancedserver.nojointimeout.field.style					= UI_SMALLFONT;
	s_advancedserver.nojointimeout.field.titleEnum				= MBT_NOJOINTIMEOUT;
	s_advancedserver.nojointimeout.field.titlecolor				= CT_LTGOLD1;
	s_advancedserver.nojointimeout.field.textcolor				= CT_DKGOLD1;
	s_advancedserver.nojointimeout.field.textcolor2				= CT_LTGOLD1;
...
//	s_advancedserver.friendlyfire.curvalue	= Com_Clamp( 0, 1, trap_Cvar_VariableValue( "g_friendlyFire" ) );
	...
	Com_sprintf( s_advancedserver.nojointimeout.field.buffer, 8, "%i", (int)Com_Clamp( 0, 600, trap_Cvar_VariableValue( "g_nojointimeout" ) ) );
