...
void forcefield_think(gentity_t *ent)
{
	if (ent->damage)
	{
		// field was just on, so think again when it's done being off
		ent->nextthink = level.time + (ent->count*1000);
		// move the field elsewhere when it's off
		VectorCopy( ent->pos2, ent->s.pos.trBase );
	...
	{
		// field was just off, so think again when it's done being on
		ent->nextthink = level.time + (ent->health*1000);
		// move the field elsewhere when it's off
		VectorCopy( ent->pos1, ent->s.pos.trBase );
		VectorCopy( ent->pos1, ent->r.currentOrigin );
		ent->damage = 1;
...
{
	// damage keeps track of whether the field is on or off
	ent->damage = 1;

	// remember our initial position
	VectorCopy(ent->s.origin, ent->pos1);

	if (ent->spawnflags & SF_USEABLE)
	{
