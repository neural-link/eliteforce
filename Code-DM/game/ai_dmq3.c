...
void BotUpdateInventory(bot_state_t *bs) {
	...
	bs->inventory[INVENTORY_STASIS] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_STASIS)) != 0;
	bs->inventory[INVENTORY_PHASER] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_PHASER)) != 0;
	bs->inventory[INVENTORY_DREADNOUGHT] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_DREADNOUGHT)) != 0;
	bs->inventory[INVENTORY_IMOD] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_IMOD)) != 0;
	bs->inventory[INVENTORY_COMPRESSION] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_COMPRESSION_RIFLE)) != 0;
	bs->inventory[INVENTORY_TETRION] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_TETRION_DISRUPTOR)) != 0;
	bs->inventory[INVENTORY_SCAVENGER] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_SCAVENGER_RIFLE)) != 0;
	...
	bs->inventory[INVENTORY_STASISAMMO] = bs->cur_ps.ammo[WP_STASIS];
	bs->inventory[INVENTORY_PHASERAMMO] = bs->cur_ps.ammo[WP_PHASER];
	bs->inventory[INVENTORY_DREADNOUGHTAMMO] = bs->cur_ps.ammo[WP_DREADNOUGHT];
	bs->inventory[INVENTORY_IMODAMMO] = bs->cur_ps.ammo[WP_IMOD];
	bs->inventory[INVENTORY_COMPRESSIONAMMO] = bs->cur_ps.ammo[WP_COMPRESSION_RIFLE];
	bs->inventory[INVENTORY_TETRIONAMMO] = bs->cur_ps.ammo[WP_TETRION_DISRUPTOR];
	bs->inventory[INVENTORY_SCAVENGERAMMO] = bs->cur_ps.ammo[WP_SCAVENGER_RIFLE];
...
=========================
*/
#define DETPACK_RADIUS		500 // coincides with g_active.c
...
int BotCanAndWantsToRocketJump(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_STASIS] <= 0) return qfalse;
	//if low on rockets
	if (bs->inventory[INVENTORY_STASISAMMO] < 3) return qfalse;
...
int BotWantsToCamp(bot_state_t *bs) {
	...
	//the bot should have at least have the compression rifle, the IMOD or the Photon Launcher with some ammo
	if ((bs->inventory[INVENTORY_COMPRESSION] <= 0 || bs->inventory[INVENTORY_COMPRESSIONAMMO] < 10) &&
		(bs->inventory[INVENTORY_IMOD] <= 0 || bs->inventory[INVENTORY_IMODAMMO] < 10) &&
		(bs->inventory[INVENTORY_QUANTUM] <= 0 || bs->inventory[INVENTORY_QUANTUMAMMO] < 10)) {
		return qfalse;
	}
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
	...
	if ( bs->weaponnum == WP_BORG_ASSIMILATOR || bs->weaponnum == (WP_VOYAGER_HYPO+WP_NUM_WEAPONS) )
	{//get real close
		attack_dist = 16;
		attack_range = 16;
	}
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	if (wi.number == WP_SCAVENGER_RIFLE) {
